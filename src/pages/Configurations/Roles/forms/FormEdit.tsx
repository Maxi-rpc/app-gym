...
import { UpdateRoleInput } from "../../../../service/types/Role";
import { rolesService } from "../../../../service/role.service";

type Props = {
	onSubmit?: () => void;
	onClose?: () => void;
	defaultData: UpdateRoleInput | null;
};
...
export default function FormEdit({ onSubmit, onClose, defaultData }: Props) {
	...
	const handleSubmit = async () => {
		try {
			...
			const resp = await rolesService.update(formData);
			if (resp.error) {
				throw resp.error;
			}
			...
			console.error("Error al guardar role:", error);

			setFeedback({
				variant: "error",
				title: "No se puede guardar role",
				message:
