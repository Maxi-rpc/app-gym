...
import { UpdateMembershipStatusInput } from "../../../../service/types/MembershipStatus";
import { membershipStatusService } from "../../../../service/membershipStatus.service";

type Props = {
	onSubmit?: () => void;
	onClose?: () => void;
	defaultData: UpdateMembershipStatusInput | null;
};
...
export default function FormEdit({ onSubmit, onClose, defaultData }: Props) {
	...
	const handleSubmit = async () => {
		try {
			...
			const resp = await membershipStatusService.update(formData);
			if (resp.error) {
				throw resp.error;
			}
			...
			setFeedback({
				variant: "error",
				title: "No se puede guardar estado",
				message:
