...
import { membershipStatusService } from "../../../service/membershipStatus.service";

import DataTable from "./DataTable";
import ModalAdd from "./modals/ModalAdd";
import ModalEdit from "./modals/ModalEdit";

export default function MembershipsStatus() {
	...
	const [selectData, setSelectData] = useState<MembershipStatus | null>(null);
	const [listData, setListData] = useState<MembershipStatus[] | []>([]);
	...
	const getData = async () => {
		try {
			...
			const resp = await membershipStatusService.getAll();
			if (resp.error) {
				throw resp.error;
			}
	...
	return (
		<div>
			...
			/>
			<PageBreadcrumb pageTitle="Membership Status" />
			<div className="rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 xl:px-10 xl:py-12">
				<div className="mx-auto w-full text-center mb-8">
					<h3 className="mb-4 font-semibold text-gray-800 text-theme-xl dark:text-white/90 sm:text-2xl">
						Listado de Estado de Membresía
					</h3>

					<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
						Se muestran los estados de membresía de la app, se puede Agregar o
						Editar.
					</p>
