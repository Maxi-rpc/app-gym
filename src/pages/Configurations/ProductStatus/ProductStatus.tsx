...
export default function ProductStatusView() {
	...
	const [selectData, setSelectData] = useState<ProductStatus | null>(null);
	const [listData, setListData] = useState<ProductStatus[] | []>([]);
	...
	const getData = async () => {
		try {
			...
			const resp = await productStatusService.getAll();
			if (resp.error) {
				throw resp.error;
			}
	...
	return (
		<div>
			...
			/>
			<PageBreadcrumb pageTitle="Product Status" />
			<div className="rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 xl:px-10 xl:py-12">
				<div className="mx-auto w-full text-center mb-8">
					<h3 className="mb-4 font-semibold text-gray-800 text-theme-xl dark:text-white/90 sm:text-2xl">
						Listado de Estados de Productos
					</h3>

					<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
						Se muestran los estados de productos de la app, se puede Agregar o
						Editar.
					</p>
				...
				<div className="flex flex-col md:flex-row justify-between md:items-end gap-4 max-sm:px-4 mb-3">
					<div className="space-y-6 w-full">
						<Label htmlFor="inputTwo">Buscar</Label>
						<Input
							type="text"
							id="inputTwo"
							placeholder="nombre"
