...
type Props = {
	...
	searchText: string;
	onEdit?: (data: ProductStatus) => void;
};
...
export default function DataTable({ listData, searchText, onEdit }: Props) {
	...
	const handleEdit = (data: ProductStatus) => {
		onEdit?.(data);
	};

	const filterData = (listData: ProductStatus[]) => {
		if (!searchText.trim()) {
			return listData;
		}
	...
	const getSortValue = (data: ProductStatus, key: SortKey): string | number => {
		switch (key) {
			case "id":
				return data.id ?? "";
			case "createdAt":
