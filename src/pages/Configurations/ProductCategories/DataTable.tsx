...
type Props = {
	...
	searchText: string;
	onEdit?: (data: ProductCategories) => void;
};
...
export default function DataTable({ listData, searchText, onEdit }: Props) {
	...
	const handleEdit = (data: ProductCategories) => {
		onEdit?.(data);
	};

	const filterData = (listData: ProductCategories[]) => {
		if (!searchText.trim()) {
			return listData;
		}
	...
	const getSortValue = (data: ProductCategories, key: SortKey): string | number => {
		switch (key) {
			case "id":
				return data.id ?? "";
			case "createdAt":
