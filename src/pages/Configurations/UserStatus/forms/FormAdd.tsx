...
export default function FormAdd({ onSubmit, onClose }: Props) {
	...
	const handleSubmit = async () => {
		try {
			...
			const resp = await userStatusService.create(formData);
			if (resp.error) {
				throw resp.error;
			}
			...
			setFeedback({
				variant: "success",
				title: "User Status creado.",
				message: resp?.data?.message,
			});
		} catch (error) {
			console.error("Error al crear user status:", error);

			setFeedback({
				variant: "error",
				title: "No se puede crear user status",
				message:
