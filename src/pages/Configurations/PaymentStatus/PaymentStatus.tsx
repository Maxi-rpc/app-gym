...
import { PaymentStatus } from "../../../service/types/PaymentsStatus";
...
export default function Roles() {
	...
	const [selectData, setSelectData] = useState<PaymentStatus | null>(null);
	const [listData, setListData] = useState<PaymentStatus[] | []>([]);

	const getData = async () => {
		try {
			setFeedback(null);

			const resp = await paymentStatusService.getAll();
			if (resp.error) {
				throw resp.error;
			}
			...
			console.error("Error al obtener payments status:", error);

			setFeedback({
				variant: "error",
				title: "No se pudieron cargar los payments status",
				message:
	...
	const handleEdit = (data: PaymentStatus) => {
		setSelectData(data);
		openModalEdit();
	};
	...
	return (
		<div>
			...
			/>
			<PageBreadcrumb pageTitle="Payment Status" />
			<div className="rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 xl:px-10 xl:py-12">
				<div className="mx-auto w-full text-center mb-8">
					<h3 className="mb-4 font-semibold text-gray-800 text-theme-xl dark:text-white/90 sm:text-2xl">
						Listado de Estado de Pagos
					</h3>

					<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
						Se muestran los estados de pago de la app, se puede Agregar o
						Editar.
					</p>
