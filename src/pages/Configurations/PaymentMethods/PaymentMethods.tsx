...
export default function Roles() {
	...
	const [selectData, setSelectData] = useState<PaymentMethods | null>(null);
	const [listData, setListData] = useState<PaymentMethods[] | []>([]);

	const getData = async () => {
		try {
			setFeedback(null);

			const resp = await paymentMethodsService.getAll();
			if (resp.error) {
				throw resp.error;
			}
	...
	const handleEdit = (data: PaymentMethods) => {
		setSelectData(data);
		openModalEdit();
	};
			...
			<div className="rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/3 xl:px-10 xl:py-12">
				<div className="mx-auto w-full text-center mb-8">
					<h3 className="mb-4 font-semibold text-gray-800 text-theme-xl dark:text-white/90 sm:text-2xl">
						Listado de Método de Pagos
					</h3>

					<p className="text-sm text-gray-500 dark:text-gray-400 sm:text-base">
						Se muestran los métodos de pago de la app, se puede Agregar o
						Editar.
					</p>
				...
				<div className="flex flex-col md:flex-row justify-between md:items-end gap-4 max-sm:px-4 mb-3">
					<div className="space-y-6 w-full">
						<Label htmlFor="inputTwo">Buscar Método</Label>
						<Input
							type="text"
							id="inputTwo"
							placeholder="nombre"
